"""
This script is intended to be used with the related filter_finder.py. 
It filters out all the pixels corresponding to a single skateboard in a 
given controlled video of a skateboard under static conditions.

TODO: make this script configurable to run from the commandline with no 
need to edit code, e.g. through `python board_filter -v "board.mp4" -c 
"wheels"`
"""
...
# lower and upper bounds for color detections, to be derived 
# using keyboard controls. The default values given here are the 
# averages of previous trials with Steve's skateboard.
...
#factor by which to scale video down
downsize_scale = 2

path = '[filename.ext]' # TODO: make path passable from the commandline
...
if os.path.isfile(path):
	...
	if success:
		...
		while success:
			...
			frame = cv2.resize(frame, (width / downsize_scale, height / downsize_scale))
