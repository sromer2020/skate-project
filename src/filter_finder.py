...
class FilterFinder:
    ...
    #get nicely-formatted params
    def export_formatted_params(self):
        export = '{\r\n'
        for key, lists in self.filters.iteritems():
            export = export + '\t\'{0}\': ['.format(key)
            for paramset in lists:
                export = export + 'np.array(['
                for param in paramset:
                    export = export + str(param) + ', '
                export = export[:-2] + ']), '
            export = export[:-2] + '],\r\n'
        export = export[:-3] + '}'
        return export
